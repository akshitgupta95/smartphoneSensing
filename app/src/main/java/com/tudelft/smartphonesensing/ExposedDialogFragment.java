...
import com.tudelft.smartphonesensing.util.DatePickerFragmentDialog;
...
public class ExposedDialogFragment extends DialogFragment {
	...
	private void checkAndDismissDialog(long selectedDate, String codeInput) {
		...
		if (!dateValid) {
			errorView.setText("Invalid code! Please check again.");
			errorView.setVisibility(View.VISIBLE);
			return;
		}
		boolean codeValid = authCodeRegex == null || codeInput.matches(authCodeRegex);
		if (!codeValid) {
			errorView.setText("Invalid date! Please check again.");
			errorView.setVisibility(View.VISIBLE);
			return;
		}
